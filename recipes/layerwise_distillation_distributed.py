...
class DistillationRecipeDistributed(FTRecipeInterface):
    ...
    def _setup_models(
        ...
        if enable_activation_checkpointing and ac_mode is None:
            utils.set_activation_checkpointing(
                teacher_model, auto_wrap_policy={modules.TransformerDecoderLayer}
            )
            utils.set_activation_checkpointing(
    ...
    def save_checkpoint(self, epoch: int) -> None:
        ...
        with FSDP.state_dict_type(
            ...
            StateDictConfig(offload_to_cpu=True, rank0_only=True),
            OptimStateDictConfig(offload_to_cpu=True, rank0_only=True),
        ):
    ...
    def train(self) -> None:
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                for layer_idx, layer in enumerate(self._student_model.layers):
                    teacher_activations = self._teacher_model.layers[layer_idx](
                        teacher_inputs
                    )
                    teacher_activations = teacher_activations.detach()
                    student_activations = layer(teacher_inputs)

                    loss = self._loss_fn(student_activations, teacher_activations)
                    running_loss += loss.item()
                    loss.backward()
                    self._optimizers[layer_idx].step()
                    teacher_inputs = teacher_activations
